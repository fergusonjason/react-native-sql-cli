...
const main = async function(projectName, commanderOptions) {
    ...
    if (commanderOptions.lint) {
        if (answers.jsLint) {
            installJsLintAndStyles(answers.jsLintType);
        }
    }
...
function buildQuestions(projectName, commanderOptions) {
    ...
    if (commanderOptions.lint) {
        ...
        questions.push({type: 'confirm', name: 'jsLint', message:'Do you wish to install JSlint', default: true});
        questions.push({type: 'list', name: 'jsLintType', message: 'Which lint style do you want?', when: function(response) {return response.jsLint;}, choices: lintTypes});
    }

    if (commanderOptions.babel) {
        questions.push({type: 'confirm', name: 'babel', message: 'Do you wish to install Babel and the RN preset?', default: true});
    }
...
function installJsLintAndStyles(type) {
    ...
    shelljs.echo("Installing eslint");
    shelljs.exec("yarn add eslint --dev --silent");
    ...
    if (installPackage) {
        if (Array.isArray(installPackage)) {
            installPackage.forEach(ip => {
                shelljs.echo("Installing esLint style: " + type);
                shelljs.exec("yarn add " + ip + " --dev --silent");
            });
        } else {
            shelljs.echo("Installing esLint style: " + type);
            shelljs.exec("yarn add " + installPackage + " --dev --silent");
        }
    }
